...
export default function EmployeeShell({ children }) {
  ...
  return <div className="font-dashboard min-h-screen bg-[#f7f9fb] text-slate-900">
    <aside className="fixed left-0 top-0 z-50 hidden h-screen w-[280px] flex-col border-r border-slate-200 bg-white shadow-sm md:flex">
      <Link href="/dashboard" className="mb-2 flex items-center gap-3 px-6 py-6"><img src="/images/logo.png" alt="Sebisa Presensi" className="h-20 w-20 shrink-0 object-contain" /><div><h1 className="text-base font-bold leading-tight text-slate-900">Sebisa Presensi</h1><p className="text-xs font-medium text-slate-500">Employee Portal</p></div></Link>
      ...
      <div className="mt-auto border-t border-slate-200 px-3 py-4"><Link href="/profile" className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-100"><span className="material-symbols-outlined">person</span>Profil</Link><button onClick={logout} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-red-50 hover:text-red-600">{icon('logout')}Keluar</button></div>
    </aside>
    <header className="fixed right-0 top-0 z-40 hidden h-16 w-[calc(100%-280px)] items-center justify-end border-b border-slate-200 bg-slate-50/80 px-8 backdrop-blur-md md:flex"><div className="flex items-center gap-4"><span className="material-symbols-outlined text-slate-500">notifications</span><div className="h-8 w-px bg-slate-200" /><div className="relative"><button onClick={() => setProfileOpen(!profileOpen)} className="flex items-center gap-3"><div className="text-right"><p className="text-sm font-semibold text-slate-900">{user?.name || 'Memuat...'}</p><p className="text-xs capitalize text-slate-500">{user?.role || 'employee'}</p></div><span className="flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 bg-slate-100 text-xs font-bold text-slate-700">{user?.name?.slice(0, 2).toUpperCase() || '--'}</span>{icon('expand_more', 18)}</button>{profileOpen && <div className="absolute right-0 mt-2 w-56 rounded-xl border border-slate-200 bg-white py-2 shadow-lg"><div className="border-b border-slate-100 px-4 py-3"><p className="text-sm font-semibold">{user?.name}</p><p className="text-xs text-slate-500">{user?.email}</p></div><Link href="/profile" className="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-100">{icon('person', 18)}Profil & Password</Link></div>}</div></div></header>
