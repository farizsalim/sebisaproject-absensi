...
import { formatDateOnly, formatTime, formatTimeInput } from '@/lib/utils'
  ...
  return <RoleShell user={user}>
    ...
    <section className="mb-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <form onSubmit={submit} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        <label htmlFor="attendance-employee" className="text-xs font-semibold text-slate-700">Employee<select id="attendance-employee" required disabled={Boolean(editing)} value={form.employeeId} onChange={(event) => setForm({ ...form, employeeId: event.target.value })} className="field mt-1 disabled:bg-slate-100"><option value="">Pilih employee</option>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.fullName} · {employee.employeeNumber}</option>)}</select></label>
        <label htmlFor="attendance-date" className="text-xs font-semibold text-slate-700">Tanggal<input id="attendance-date" required type="date" value={form.attendanceDate} onChange={(event) => setForm({ ...form, attendanceDate: event.target.value })} className="field mt-1" /></label>
        <TimePicker id="attendance-clock-in" label="Jam masuk" value={form.clockInAt} onChange={(clockInAt) => setForm({ ...form, clockInAt })} />
        <TimePicker id="attendance-clock-out" label="Jam pulang" value={form.clockOutAt} onChange={(clockOutAt) => setForm({ ...form, clockOutAt })} />
        <label htmlFor="attendance-reason" className="text-xs font-semibold text-slate-700">Alasan perubahan (opsional)<input id="attendance-reason" placeholder="Tulis alasan" value={form.reason} onChange={(event) => setForm({ ...form, reason: event.target.value })} className="field mt-1" /></label>
