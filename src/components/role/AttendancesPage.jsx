...
export default function AttendancesPage() {
  ...
  const loadData = (date = filterDate) => Promise.all([fetch(`/api/hr/attendances?date=${date}`), fetch('/api/hr/employees')]).then(async ([attendanceResponse, employeeResponse]) => {
  ...
  return <RoleShell user={user}>
    ...
    <section className="mb-6 flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-end sm:justify-between">
      <div><label htmlFor="attendance-filter-date" className="text-sm font-semibold text-slate-700">Filter tanggal presensi</label><input id="attendance-filter-date" type="date" value={filterDate} onChange={(event) => setFilterDate(event.target.value)} className="field mt-2 block w-full sm:w-56" /></div>
      <p className="text-sm text-slate-500">Menampilkan presensi pada tanggal yang dipilih.</p>
    </section>
    <section className="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm"><table className="w-full min-w-[820px] text-left text-sm"><thead className="border-b border-slate-200 bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-5 py-4">Tanggal</th><th className="px-5 py-4">Employee</th><th className="px-5 py-4">Masuk</th><th className="px-5 py-4">Pulang</th><th className="px-5 py-4">Status</th><th className="px-5 py-4">Sumber</th><th className="px-5 py-4">Aksi</th></tr></thead><tbody className="divide-y divide-slate-100">{attendances.map((item) => <tr key={item.id}><td className="px-5 py-4">{new Date(item.attendanceDate).toLocaleDateString('id-ID')}</td><td className="px-5 py-4 font-semibold">{item.employee?.fullName || '-'}</td><td className="px-5 py-4">{formatTime(item.clockInAt)}</td><td className="px-5 py-4">{formatTime(item.clockOutAt)}</td><td className="px-5 py-4 capitalize">{item.status}</td><td className="px-5 py-4 capitalize">{item.source}</td><td className="space-x-2 px-5 py-4"><button onClick={() => edit(item)} className="rounded-lg bg-slate-900 px-3 py-2 text-xs font-semibold text-white hover:bg-slate-800">Edit</button><button onClick={() => viewAudit(item)} className="rounded-lg bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-200">Audit</button></td></tr>)}</tbody></table>{attendances.length === 0 && <p className="p-10 text-center text-sm text-slate-500">Belum ada data presensi.</p>}</section>
