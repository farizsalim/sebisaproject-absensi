...
export default function RoleShell({ children, user, leader = false }) {
  ...
  return <div className="min-h-screen bg-[#f7f9fb] text-slate-900">
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[280px] flex-col border-r border-slate-200 bg-white md:flex">
      <Link href="/console" className="flex items-center gap-3 px-6 py-6"><img src="/images/logo.png" alt="Sebisa Presensi" className="h-16 w-16 object-contain" /><div><strong className="block text-base">Sebisa Presensi</strong><span className="text-xs text-slate-500">Console Management System</span></div></Link>
      ...
      <div className="border-t border-slate-200 px-3 py-4"><Link href="/profile" className={`mb-2 flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold ${isActive('/profile') ? 'bg-emerald-100 text-emerald-800' : 'text-slate-600 hover:bg-slate-100'}`}>{icon('person')}Profil</Link><button onClick={logout} className="flex w-full gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold text-slate-600 hover:bg-rose-50 hover:text-rose-600">{icon('logout')}Keluar</button></div>
    </aside>
    <header className="fixed right-0 top-0 z-30 hidden h-16 w-[calc(100%-280px)] items-center justify-end border-b border-slate-200 bg-slate-50/80 px-8 backdrop-blur-md md:flex"><div className="flex items-center gap-4"><span className="material-symbols-outlined text-slate-500">notifications</span><div className="h-8 w-px bg-slate-200" /><div className="relative"><button onClick={() => setProfileOpen(!profileOpen)} className="flex items-center gap-3"><div className="text-right"><p className="text-sm font-semibold text-slate-900">{currentUser?.name || 'Memuat...'}</p><p className="text-xs uppercase text-slate-500">{currentUser?.role || 'hr'}</p></div><span className="flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 bg-slate-100 text-xs font-bold text-slate-700">{currentUser?.name?.slice(0, 2).toUpperCase() || '--'}</span>{icon('expand_more', 18)}</button>{profileOpen && <div className="absolute right-0 mt-2 w-56 rounded-xl border border-slate-200 bg-white py-2 shadow-lg"><div className="border-b border-slate-100 px-4 py-3"><p className="text-sm font-semibold">{currentUser?.name}</p><p className="text-xs text-slate-500">{currentUser?.email}</p></div><Link href="/profile" className="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-100">{icon('person', 18)}Profil & Password</Link></div>}</div></div></header>
