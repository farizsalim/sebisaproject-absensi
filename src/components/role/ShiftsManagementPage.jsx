...
function TimePicker({ label, value, onChange }) {
  ...
  return <div className="relative text-sm font-semibold text-slate-700"><span>{label}</span><button type="button" onClick={() => setOpen(!open)} className="field mt-1 flex w-full items-center justify-between text-left font-normal"><span className={value ? 'text-slate-900' : 'text-slate-400'}>{value || 'Pilih jam'}</span><span className="material-symbols-outlined text-[18px] text-slate-500">schedule</span></button>{open && <div className="absolute left-0 top-full z-20 mt-2 w-full min-w-[250px] rounded-xl border border-slate-200 bg-white p-3 shadow-xl"><div className="grid grid-cols-2 gap-3"><div><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Jam</p><div className="grid max-h-48 grid-cols-1 gap-1 overflow-y-auto">{Array.from({ length: 24 }, (_, hour) => { const item = String(hour).padStart(2, '0'); return <button type="button" key={item} onClick={() => choose('hours', item)} className={`rounded-md px-1 py-2 text-xs font-semibold ${hours === item ? 'bg-emerald-600 text-white' : 'text-slate-700 hover:bg-emerald-50'}`}>{item}</button> })}</div></div><div><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Menit</p><div className="grid max-h-48 grid-cols-1 gap-1 overflow-y-auto">{Array.from({ length: 60 }, (_, minute) => { const item = String(minute).padStart(2, '0'); return <button type="button" key={item} onClick={() => choose('minutes', item)} className={`rounded-md px-1 py-2 text-xs font-semibold ${minutes === item ? 'bg-emerald-600 text-white' : 'text-slate-700 hover:bg-emerald-50'}`}>{item}</button> })}</div></div></div><button type="button" onClick={() => setOpen(false)} className="mt-3 w-full rounded-lg bg-slate-900 px-3 py-2 text-xs font-semibold text-white hover:bg-slate-800">Selesai</button></div>}</div>
}
  ...
  return <RoleShell>
    ...
    <section className="mb-6 rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
      ...
      <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <TimePicker label="Batas clock in" value={form.clockInDeadline} onChange={(clockInDeadline) => setForm({ ...form, clockInDeadline })} />
        <TimePicker label="Batas clock out" value={form.clockOutDeadline} onChange={(clockOutDeadline) => setForm({ ...form, clockOutDeadline })} />
        ...
        <div className="flex items-end sm:col-span-2 lg:col-span-4"><button disabled={saving} className="min-h-11 w-full rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700 disabled:opacity-50 sm:w-auto">{saving ? 'Menyimpan...' : editing ? 'Simpan perubahan' : 'Tambah shift'}</button></div>
      </form>
    </section>
