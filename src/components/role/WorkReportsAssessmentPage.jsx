...
export default function WorkReportsAssessmentPage() {
  ...
  return <RoleShell>
    ...
    {loading ? <div className="rounded-xl border border-slate-200 bg-white p-10 text-center text-sm text-slate-500">Memuat laporan...</div> : reports.length === 0 ? <div className="rounded-xl border border-slate-200 bg-white p-10 text-center text-sm text-slate-500">Belum ada laporan kerja.</div> : <section className="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm"><table className="w-full min-w-[900px] text-left text-xs sm:text-sm [&_th]:px-3 [&_th]:py-3 sm:[&_th]:px-5 sm:[&_th]:py-4 [&_td]:px-3 [&_td]:py-3 sm:[&_td]:px-5 sm:[&_td]:py-4"><thead className="border-b border-slate-200 bg-slate-50 text-[10px] uppercase tracking-wide text-slate-500 sm:text-xs"><tr><th className="whitespace-nowrap">Employee</th><th className="whitespace-nowrap">Tanggal</th><th className="whitespace-nowrap">Laporan</th><th className="whitespace-nowrap">Nilai (0-100)</th><th className="whitespace-nowrap">Catatan HR</th><th className="whitespace-nowrap">Aksi</th></tr></thead><tbody className="divide-y divide-slate-100">{reports.map((report) => <tr key={report.id} className="align-top"><td className="max-w-[130px] break-words font-semibold">{report.employee?.fullName || '-'}</td><td className="whitespace-nowrap">{new Date(report.reportDate).toLocaleDateString('id-ID')}</td><td className="max-w-sm"><p className="font-semibold">{report.title}</p><p className="mt-1 break-words text-slate-600">{report.description}</p></td><td><input type="number" min="0" max="100" step="0.01" placeholder="0-100" value={forms[report.id]?.score || ''} onChange={(event) => updateForm(report.id, 'score', event.target.value)} className="field w-24 sm:w-28" /></td><td><textarea rows="2" placeholder="Catatan penilaian" value={forms[report.id]?.reviewNote || ''} onChange={(event) => updateForm(report.id, 'reviewNote', event.target.value)} className="field min-w-44 sm:min-w-52" /></td><td><button type="button" onClick={() => saveAssessment(report)} disabled={saving === report.id} className="min-h-10 whitespace-nowrap rounded-lg bg-emerald-600 px-3 py-2 text-xs font-semibold text-white hover:bg-emerald-700 disabled:opacity-50">{saving === report.id ? 'Menyimpan...' : 'Simpan Nilai'}</button></td></tr>)}</tbody></table></section>}
  </RoleShell>
}
